...
# Skipped atm: resources environmental subtags join, resources building blocks substeps join, cognitive bia, resource types, 
  ...
  new_resource = Resource.create(title: title, author: author, date: date, abstract: summary, url: url, admin_notes: admin_notes, world_region_id: WorldRegion.where(name: world_region).id, news_source_id: NewsSource.where(name: news_source).id, is_problem: is_problem)
  # "resource_type_id"
